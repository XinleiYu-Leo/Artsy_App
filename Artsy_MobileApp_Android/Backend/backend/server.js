  ...
  headers: {
    'X-Xapp-Token': token
        ...
        for(i; i< respond.data['_embedded']['genes'].length; i++){
            let newitem = {name:respond.data['_embedded']['genes'][i]['name'], description:respond.data['_embedded']['genes'][i]['description'], img:respond.data["_embedded"]['genes'][i]["_links"]["thumbnail"]["href"]};
            myJson.push(newitem);
        }
        res.send(myJson);
    })()
